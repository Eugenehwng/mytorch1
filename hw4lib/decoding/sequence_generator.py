...
class SequenceGenerator:
    ...
    def generate_greedy(
        ...
        while x.size(1) < self.max_length:
            if finished.all():
                break
            ...
            tok_score = log_probs.gather(1, next_tokens.unsqueeze(1)).squeeze(1)
            scores = torch.where(finished, scores, scores + tok_score)
    ...
    def generate_beam(
        ...
        beam_seqs = x.unsqueeze(1)
        V = self.tokenizer.vocab_size

        while beam_seqs.size(2) < self.max_length:
            _, beam, L = beam_seqs.shape
            rows = []
            for b in range(B):
                for k in range(beam):
                    row_1 = beam_seqs[b, k : k + 1, :]
                    if hasattr(self.score_fn, "trees"):
                        raw = self.score_fn.update_scores(
                            self.score_fn.trees[b], row_1.squeeze(0)
                        )
                        lg = raw.log_softmax(dim=-1)
                    else:
                        lg = self.score_fn(row_1)
                        if lg.dim() == 3:
                            lg = lg[:, -1, :]
                        lg = lg.squeeze(0)
                    rows.append(lg)
            logits = torch.stack(rows, dim=0).view(B, beam, V)

            for b in range(B):
                logits[b] = self._apply_repeat_penalty(
                    logits[b], beam_seqs[b], repeat_penalty
                )
            logits = self._filter_logits(
                logits.reshape(B * beam, V), temperature, 0, 1.0
            ).view(B, beam, V)
            log_probs = torch.log_softmax(logits, dim=-1)

            new_scores = beam_scores.unsqueeze(-1) + log_probs
            flat_scores = new_scores.view(B, -1)
            top_scores, top_idx = torch.topk(flat_scores, beam_width, dim=1)
            parent = top_idx // V
            tokens = top_idx % V

            batch_idx = torch.arange(B, device=device).unsqueeze(1).expand_as(parent)
            selected = beam_seqs[batch_idx, parent]
            beam_seqs = torch.cat([selected, tokens.unsqueeze(-1)], dim=-1)
            beam_scores = top_scores

        return beam_seqs, beam_scores
