...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.source_embedding = SpeechEmbedding(
            input_dim, d_model, time_reduction=time_reduction, reduction_method=reduction_method, dropout=dropout
        )
        self.target_embedding = nn.Embedding(num_classes, d_model)
        self.positional_encoding = PositionalEncoding(d_model, max_len)
        self.final_linear = nn.Linear(d_model, num_classes)
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        x_enc, x_enc_lengths = self.source_embedding(
            padded_sources, source_lengths.to(padded_sources.device)
        )
        ...
        ctc_inputs = {"log_probs": log_probs, "lengths": x_enc_lengths.to(torch.long)}
