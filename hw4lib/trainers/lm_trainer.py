...
class LMTrainer(BaseTrainer):
    ...
    def generate(self, dataloader, generation_config: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        max_len_gen = min(
            int(generation_config.get('max_length', self.model.max_len)),
            self.model.max_len,
        )
        ...
        if prompts.size(0) == 0:
            print("Warning: No prompts sampled for generation (dataset may be too small for prompt_length).")
            return []
        prompts = prompts.to(self.device)

        temperature = float(generation_config.get('temperature', 1.0))
        repeat_penalty = float(generation_config.get('repeat_penalty', 1.0))
        beam_width = int(generation_config.get('beam_width', 1))
        top_k = int(generation_config.get('top_k', 0))
        top_p = float(generation_config.get('top_p', 0.0))
